...
def test_users_get_response():
    response = [
        {"id": 141414, "first_name": "George", "last_name": "Stivenson"},
...
def test_users_get_success():
    ...
    assert users[0].first_name == "George"
    assert users[0].last_name == "Stivenson"
...
def test_users_get_one_user():
    response = [{"id": 141414, "first_name": "George", "last_name": "Stivenson"}]
    users = [User(**user) for user in response]
    assert len(users) == 1
    assert users[0].id == 141414
    assert users[0].first_name == "George"
